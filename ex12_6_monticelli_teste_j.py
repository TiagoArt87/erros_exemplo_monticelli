...
#Teste do indice J
def teste_indice_j(estados_estimados, vet_med, dp):
    sum = 0
    n=0
    residuo = np.array(Calcula_Residuo(vet_med, vet_estados))
    while n < len(residuo):
        sum += (residuo[n]**2)/(dp[n]**2)
        n+=1
    # Calcula o valor da função objetivo, e indica se ela possui erros grosseiros
    # com base na distribuição de probabilidade chi quadrado
    # https://acervolima.com/python-teste-qui-quadrado-de-pearson/
    critical_value= chi2.ppf(q = 0.95, df = len(vet_med)-len(estados_estimados))
    
    if sum > critical_value:
        print('provavelmente a estimação abriga erros grosseiros')
    else:
        print('provavelmente a estimação não abriga erros grosseiros')

teste_indice_j(vet_estados, vet_med, des_pad)